...
public class GradeBook {
	...
	private double GetStudentAverage(int index) {
		...
		;
		int i = 0;
		if (scores[index][i] < scores[index][i + 1] && scores[index][i] < scores[index][i + 2]
				&& scores[index][i] < scores[index][i + 3]) {
			temp = scores[index][i];
		} else if (scores[index][i + 1] < scores[index][i + 2] && scores[index][i + 1] < scores[index][i + 3]
				&& scores[index][i + 1] < scores[index][i]) {
			temp = scores[index][i + 1];
		} else if (scores[index][i + 2] < scores[index][i + 3] && scores[index][i + 2] < scores[index][i]
				&& scores[index][i + 2] < scores[index][i + 1]) {
