...
public class CompletedBST<Key extends Comparable<Key>, Value> implements BST<Key, Value> {
    ...
    private Value get(Node<Key, Value> x, Key key) { // (Value val)
        if (x == null)
            return null; // not found - null, //bst ordered/heap not
        int cmp = key.compareTo(x.key); // compares given k to current node k
        if (cmp < 0)
            return get(x.left, key); // recursively search left subtree
        else if (cmp > 0)
            return get(x.right, key); // ^ right subtree
        else
            return x.val; // if key equal return node value, key=immut/value=mut
    ...
    private Node<Key, Value> delete(Node<Key, Value> x, Key key) {
        ...
        if (cmp < 0)
            ...
            if (x.right == null)
                return x.left; // return left
            if (x.left == null)
                return x.right; // return right
