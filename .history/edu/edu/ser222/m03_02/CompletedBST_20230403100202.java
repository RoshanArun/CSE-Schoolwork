...
public class CompletedBST<Key extends Comparable<Key>, Value> implements BST<Key, Value> {
    ...
    public void balance() {
        ...
        Node<Key, Value> root = balance(allKeys, 0, allKeys.size() - 1);
        this.root = root;
    }

    private Node<Key, Value> balance(LinkedList<Key> allKeys, int start, int end) {
        if (start > end)
            return null;

        int mid = (start + end) / 2;
        Node<Key, Value> node = new Node(allKeys.get(mid), this.get(allKeys.get(mid)), 1);
        node.left = balance(allKeys, start, mid - 1);
        node.right = balance(allKeys, mid + 1, end);
        return node;
    }
    ...
    public String displayLevel(Key key) {
        ...
        if (this.contains(key)) {
            ...
            return curr.val + " " + curr.left.val + " " + curr.right.val;
        }
        return "";
    }
