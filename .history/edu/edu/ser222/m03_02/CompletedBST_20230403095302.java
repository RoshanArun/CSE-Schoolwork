...
public class CompletedBST<Key extends Comparable<Key>, Value> implements BST<Key, Value> {
    ...
    public void putFast(Key key, Value val) {
        // TODO
        // I think this is the same as the normal put() method?
        root = put(root, key, val);
    }
    ...
    private void inOrder(Node<Key, Value> x, LinkedList<Node<Key, Value>> list) {
        if (x == null)
            return;
        inOrder(x.left, list);
        list.add(x);
    ...
    private Node<Key, Value> balance(LinkedList<Node<Key, Value>> list, int start, int end) {
        if (start > end)
            return null;
        int mid = (start + end) / 2;
        Node<Key, Value> x = list.get(mid);
        x.left = balance(list, start, mid - 1);
        x.right = balance(list, mid + 1, end);
        return x;
    }
