...
public class CompletedBST<Key extends Comparable<Key>, Value> implements BST<Key, Value> {
    ...
    public void balance() {
        ...
        inOrder(root, list);
        root = rebuild(list, 0, list.size() - 1);
    }

    private void inOrder(Node<Key, Value> n, LinkedList<Node<Key, Value>> list) {
        if (n == null)
            return;
        inOrder(n.left, list);
        list.add(n);
    ...
    private Node<Key, Value> rebuild(LinkedList<Node<Key, Value>> list, int start, int end) {
        if (start > end)
            return null;
        int mid = (start + end) / 2;
        Node<Key, Value> n = list.get(mid);
        n.left = rebuild(list, start, mid - 1);
        n.right = rebuild(list, mid + 1, end);
        return n;
    }

    public String displayLevel(Key key) {
        Node<Key, Value> start = get(key);
        Queue<Node<Key, Value>> q = new Queue<>();
        q.add(start);
        StringBuilder sb = new StringBuilder();
        while (!q.isEmpty()) {
            Node<Key, Value> node = q.remove();
            sb.append(node.val + " ");
            if (node.left != null)
                q.add(node.left);
            if (node.right != null)
