...
public class CompletedLinearProbingHT<Key, Value> implements ProbingHT<Key, Value> {
    ...
    public void put(Key key, Value val) {
        ...
        int i = 0;
        int hash = hash(key, i);
        while (entries[hash] != null && !check(key, hash)) {
            i++;
            hash = hash(key, i);
        }
        entries[hash] = val;
    ...
    private boolean check(Key key, int hash) {
        if (key instanceof Integer && entries[hash] instanceof Integer) {
            return (Integer) key == (Integer) entries[hash];
        } else if (key instanceof String && entries[hash] instanceof String) {
            return (String) key == (String) entries[hash];
        }
        return false;
    }
