...
public class CheckersGameLogic implements CheckersGame {
    ...
    public void doTurn() {
        ...
        char startColumn = moveInput.charAt(0);
        char startRow = moveInput.charAt(1);
        char endColumn = moveInput.charAt(3);
        char endRow = moveInput.charAt(4);

        int startIntColumn = charToInt(startColumn);
        int startIntRow = charToInt2(startRow);
        int endIntColumn = charToInt(endColumn);
        int endIntRow = charToInt2(endRow);

        if (isValidMove(startIntRow, startIntColumn, endIntRow, endIntColumn)) {
            if (turn % 2 == 0) {
                board[endIntRow][endIntColumn] = 'x';
                board[startIntRow][startIntColumn] = '_';
            } else {
                board[endIntRow][endIntColumn] = 'o';
                board[startIntRow][startIntColumn] = '_';
            }
            turn++;
        }
    ...
    private boolean isValidMove(int startRow, int startCol, int endRow, int endCol) {
        // checks if it is the same turn as the pieces color
        if ((board[startRow][startCol] == 'x' && turn % 2 == 0) || (board[startRow][startCol] == 'o'
                && turn % 2 == 1)) {
            // checks if it is a normal move
            if ((startRow == endRow - 1 && startCol == endCol - 1)
                    || (startRow == endRow - 1 && startCol == endCol + 1)
                    || (startRow == endRow + 1 && startCol == endCol - 1)
                    || (startRow == endRow + 1 && startCol == endCol + 1)) {
                // checks if it's an empty space
            ...
            // checks if it is a jump
                ...
                // checks if it's an empty space
                if (board[endRow][endCol] == '_') {
                    // checks if their is an opposite color between the initial position and the
                    // final position
                    if (board[startRow + (endRow - startRow) / 2][startCol + (endCol - startCol) / 2] == 'x') {
                        return true;
                    } else if (board[startRow + (endRow - startRow) / 2][startCol
                            + (endCol - startCol) / 2] == 'o') {
                        return true;
                    }
                }
