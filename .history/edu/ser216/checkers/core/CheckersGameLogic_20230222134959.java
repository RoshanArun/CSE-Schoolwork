...
public class CheckersGameLogic implements CheckersGame {
    ...
    public char getWinningPlayer() {
        ...
        boolean xMovesPossible = false;
        boolean oMovesPossible = false;
        int x = 0;
        int o = 0;

        // loops through board to check if any moves are possible
        for (int row = 0; row < 8; row++) {
        ...
        // determines if x or o has won
        if (xMovesPossible == false || x == 0) {
            return 'o';
        } else if (oMovesPossible == false || o == 0) {
            return 'x';
        } else {
            return '_';
        }
