...
public class CheckersGameLogic implements CheckersGame {
    ...
    public char getWinningPlayer() {
        boolean playerXCanMove = false;
        boolean playerOCanMove = false;
        ...
        for (int row = 0; row < 8; row++) {
            ...
            if (playerXCanMove || playerOCanMove) {
                break;
            }
        }
        // if true, player x wins, if false, player o wins
        if (playerXCanMove && !playerOCanMove) {
            return 'x';
        } else if (!playerXCanMove && playerOCanMove) {
            return 'o';
        }
        return '_';
    }
