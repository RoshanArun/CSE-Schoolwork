...
public class CompletedLinearProbingHT<Key, Value> implements ProbingHT<Key, Value> {
    ...
    // chaining - hash(k) = k mod 5, linear probing- hash(k, i) = (k mod 11 + i) mod
    // 11
    // Hash function coverts data of arbitrary length to a fixed length. LP:
    // requires resize
    // collision handling - chaining, linear probing, quadprobing (hash(x) + 1*1) %
    // S)
    ...
    // open-addressing - linear-probing, require only 1 operation, chaining: large
    // data sets
    // when HashTable gets full, collisions happen more often (high load factor)
    // probing requires you to check more Hashtable locations before finding the
    // value needed.
    // At load factor of 0.8, chaining becomes more efficient due to multiple
    // collisions:
    // you would have to probe a lot of empty cells in order to find the actual
    // value you want
    // with probing, while with chaining you have a list of values that have the
    // same hash key.
    ...
    public Value get(Key key) { // put(Value val)
        ...
        int i = 0; // put (return;)
        // put if (val == null) {delete(key); return;} if val is null delete associated
        // key
        while (i < M) {
            ...
            if (((Entry<Key, Value>) entries[h]).key.equals(key)) {
                ...
                // put ((Entry<Key, Value>) entries[h]).val = val; break; // If key at index
                // equals key update value
