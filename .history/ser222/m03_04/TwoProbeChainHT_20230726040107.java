...
/**
 * TwoProbeChainHT defines an interface to a two probe chaining implementation
 * of SymbolTable that
 * offers methods specific to a two probe chaining hashtable.
 *
 * Please note that these methods are only intended to enable effective grading.
 * In practice, these
 * methods would be omitted from a hashtable ADT since they expose internal
 * state.
 *
 * @author Acuna
 * @version 1.0
...
public interface TwoProbeChainHT<Key, Value> extends SymbolTable<Key, Value> {
    ...
    /**
     * Returns the size of the chain at an index in the internal array. For example:
     * if an index is
     * unused, it will return 0. If two key/value pairs have been added at the
