...
void readPixelsBMP(FILE *file, struct Pixel **pArr, int width, int height)
{
	...
	*pArr = (struct Pixel *)malloc(sizeof(struct Pixel) * width * height);
	...
	for (int x = 0; x < width; x++)
	{
		...
		{
			fread(&temp[count].b, sizeof(char), 1, file);
			fread(&temp[count].g, sizeof(char), 1, file);
			fread(&temp[count].r, sizeof(char), 1, file);
			count++;
		}
	}
}
...
void writePixelsBMP(FILE *file, struct Pixel **pArr, int width, int height)
{
	...
	for (int y = 0; y < height; y++)
	{
		for (int x = 0; x < width; x++)
		{
			fwrite(&temp[count].b, sizeof(char), 1, file);
			fwrite(&temp[count].g, sizeof(char), 1, file);
			fwrite(&temp[count].r, sizeof(char), 1, file);
			count++;
		}
		unsigned char paddingData[4] = {0};
		fwrite(paddingData, sizeof(unsigned char),
			   (4 - (width * sizeof(struct Pixel)) % 4) % 4, file); 
	}
}