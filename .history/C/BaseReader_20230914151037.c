...
typedef struct ScoreStruct {int studentNo;   int assignNo;   float score;   char* comment;} ScoreStruct;

typedef struct Assign{int no; char* name;} Assign;

typedef struct Student{int no; char* name; AcademicLevel level;} Student;

typedef struct Course{int no; char* name; char* teacher; int numAssigns; Assign* assigns; ScoreStruct** scores;} Course;
...
//read the student names from file
        students = readStudents( file ); 
//read the course information from file
        courses = readCourses( file ); 
//read the assignment information from file
