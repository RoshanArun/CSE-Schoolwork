...
// Use pointer arithmetic to access individual characters but
...
for(int i=0; i < NUMBER_OF_CHUNKS; i++){
        char* chunkPointer = chunks[i];
        //Exclude text up to null
        while(*chunkPointer != '\0'){
            //subtract 5 from currently pointed address
            *chunkPointer = *chunkPointer - DECRYPTION_SHIFT;
