...
ScoreStruct **readScores(FILE *file, int numAssigns)
{
    ...
    for (int j = 0; j < numAssigns * numStudents; j++)
    {
        ...
        scores[studentNo - 1][assignNo - 1].studentNum = studentNo;
        scores[studentNo - 1][assignNo - 1].assignNum = assignNo;
...
void printStudents()
{
    ...
    for (int i = 0; i < numStudents; i++)
    {
        printf("  %d %s (%s)\n", students[i].num, students[i].name, getLevelString(students[i].level));
    }
}
...
void printGrades(ScoreStruct **scores, int numAssigns)
{
    ...
    for (int i = 0; i < numStudents; i++)
    {
        ...
        {
            printf("  %d %d %.02f %s\n", scores[i][j].studentNum, scores[i][j].assignNum, scores[i][j].score, scores[i][j].comment);
        }
    }
}
...
void assignmentMenu(Course course)
{
    ...
    scanf(" %d", &assignNum);
    while (assignNum < 0 || assignNum > course.numAssigns)
    {
        scanf(" %d", &assignNum);
    }
    if (assignNum == 0)
    ...
    {
        getAssignmentScore(course, assignNum);
    }
}
...
void getStudentScores(Course course, int studentNo)
{
    ...
    printf("-----------------------------------------------\n");
    for (int i = 0; i < course.numAssigns; i++)
    {
